...
export const usePerformanceAppraisal = (tasks, date) => {
    ...
    const completed = tasks.filter(tasks => tasks.conclusao === "concluida").length;
    let performance = 0;
    ...
    return {
        ...
        tasksCompleted: tasks.filter(tasks => tasks.conclusao === "concluida").length,
        tasksPending: tasks.filter(tasks => tasks.conclusao === "Pendente").length,
        tasksIncomplete: tasks.filter(tasks => tasks.conclusao === "Incompleta").length,
        tasks: tasks
    };
}