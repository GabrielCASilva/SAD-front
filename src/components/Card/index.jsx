...
export default function Card(props) {
	...
	return (
		<AntdCard
			className="w-100 h-100 card p-none f-size-16"
			tabList={tabList}
...
export function SimpleCard(props) {
	...
	return (
		<AntdCard className={`w-100 card f-size-16 ${cardClasses}`}>
			<div className={`flex column gap-14 ${containerClasses}`}>{children}</div>
		</AntdCard>
	);
}
...
export function CardRow(props) {
	...
	return <div className={`flex column gap-24 ${classes}`}>{children}</div>;
}
